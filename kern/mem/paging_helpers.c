...
//REMEMBER: to invalidate the TLB cache
 void pt_set_page_permissions(uint32* directory, uint32 virtual_address, uint32 permissions_to_set, uint32 permissions_to_clear)
{
	//[1] Get the table
...
//If the page table not exist, return -1
 int pt_get_page_permissions(uint32* directory, uint32 virtual_address )
{
...
//It's expected that the page table already exist. If not, the function should panic
//REMEMBER: to invalidate the TLB cache
 void pt_clear_page_table_entry(uint32* directory, uint32 virtual_address)
{
	//TODO: PRACTICE: fill this function.
	//Comment the following line
	panic("Function is not implemented yet!");
...
//If the page or the page table is not present, return -1
 uint32 virtual_to_physical(uint32* directory, uint32 virtual_address)
{
	//TODO: PRACTICE: fill this function.
	//Comment the following line
	panic("Function is not implemented yet!");
...
//If not found, return 0xFFFFFFFF
 uint32 physical_to_virtual(uint32* directory, uint32 physical_address)
{
	//TODO: PRACTICE: fill this function.
	//Comment the following line
	panic("Function is not implemented yet!");
...
//HINT: remember to free the allocated frame if there is no space for the necessary page table
 int alloc_page(uint32* directory, uint32 va, uint32 perms, bool set_to_zero)
{
...
/// Dealing with page directory entry flags
 ...
 uint32 pd_is_table_used(uint32* directory, uint32 virtual_address)
{
	return ( (directory[PDX(virtual_address)] & PERM_USED) == PERM_USED ? 1 : 0);
}

 void pd_set_table_unused(uint32* directory, uint32 virtual_address)
{
	directory[PDX(virtual_address)] &= (~PERM_USED);
	tlb_invalidate((void *)NULL, (void *)virtual_address);
}

 void pd_clear_page_dir_entry(uint32* directory, uint32 virtual_address)
{
